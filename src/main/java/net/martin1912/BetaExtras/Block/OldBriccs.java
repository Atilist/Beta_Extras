...
public class OldBriccs extends TemplateBlockBase {
    ...
    public boolean canUse(Level level, int x, int y, int z, PlayerBase player) {
        if (((BlockStateView) level).getBlockState(x, y, z).get(OldBriccs.METASUBSTITUTE) < 13)
            ((BlockStateView) level).setBlockState(x, y, z, BlockListener.oldBriccs.getDefaultState().with(OldBriccs.METASUBSTITUTE, ((BlockStateView) level).getBlockState(x, y, z).get(OldBriccs.METASUBSTITUTE) + 1));
        else
            ((BlockStateView) level).setBlockState(x, y, z, BlockListener.oldBriccs.getDefaultState().with(OldBriccs.METASUBSTITUTE, 0));
        return true;
    }
